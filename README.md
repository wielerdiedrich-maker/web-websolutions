...
This repo includes `render.yaml` (a Render "Blueprint") so Render can pick up
the whole config automatically from GitHub.

1. Push this branch (or merge it into `main`) to GitHub — already done if
   you're reading this from the repo.
2. On [render.com](https://render.com), sign up/log in, then
   **New → Blueprint**, and connect the `web-websolutions` GitHub repo.
   Render reads `render.yaml` and proposes one web service.
3. When prompted for environment variables it can't auto-generate
   (`ADMIN_USERNAME`, `ADMIN_PASSWORD_HASH`, and the optional `SMTP_*`
   ones), fill them in:
   ```bash
   # run locally first to generate the hash
   npm install
   npm run hash-password -- "YourStrongPassword"
   ```
   `SESSION_SECRET` is auto-generated by Render — you don't need to set it.
4. Deploy. Render builds with `npm install` and runs `npm start`. Your site
   will be live at the `*.onrender.com` URL Render gives you (or a custom
   domain you attach in the service's Settings).
5. Visit `https://<your-app>.onrender.com/admin` to log in.

**Free-tier storage caveat:** Render's free plan has no persistent disk —
the filesystem resets on every deploy, and the service spins down after
~15 minutes of inactivity, resetting again on the next visit. That means
uploaded media, the SQLite database (leads, slot assignments), and
sessions can all be wiped periodically. Fine for testing the site live;
if you want uploads and messages to actually stick around, upgrade to
Render's Starter plan and attach a persistent disk mounted at `/opt/render/project/src/data`
and `/opt/render/project/src/uploads` (or migrate storage to an object
store like Cloudflare R2/S3 — ask me when you're ready for that).
